...
- Données d'utilisation anonymisées via PostHog (sans cookies, sans identifiant persistant — voir §5)
...
stampeo utilise PostHog (hébergé dans l'UE) pour les statistiques du site. PostHog est configuré sans identifiant persistant : **aucun cookie de suivi n'est déposé et aucun identifiant n'est conservé dans le stockage du navigateur**. Les événements sont limités à la session de navigation en cours et ne sont pas corrélés d'une visite à l'autre. Aucune donnée personnelle n'est collectée à des fins analytiques. Ce mode de fonctionnement ne nécessite pas de bannière de consentement conformément à la directive ePrivacy et aux recommandations de la CNIL.
