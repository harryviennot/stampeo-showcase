# Terms of Service — Stampéo

**Last updated: February 16, 2026**

## 1. Purpose

These Terms of Service (hereinafter "Terms") govern access to and use of the Stampéo platform (hereinafter "the Platform"), a digital loyalty card service for Apple Wallet and Google Wallet.

The Platform is operated by Harry Viennot, sole proprietor in the process of registration in France, hereinafter "Stampéo", "we", or "our".

By using the Platform, you agree to these Terms in their entirety.

## 2. Definitions

- **Platform**: all Stampéo services, including the website, dashboard, mobile scanner app, and associated APIs.
- **Business User**: any individual or legal entity creating a professional account on the Platform to manage a loyalty program.
- **Employee**: any person invited by a Business User to scan customer loyalty cards.
- **End Customer**: any person holding a digital loyalty card in their wallet, as a customer of a Business User.
- **Pass**: the digital loyalty card installed in Apple Wallet or Google Wallet.
- **Subscription**: the paid plan providing access to Platform features.

## 3. Access to the Platform

### 3.1 Business Users

Registration is open to any individual of legal age (18 or older) or legal entity. Business Users agree to provide accurate information and keep it up to date.

### 3.2 Employees

Employees are invited by a Business User. They must create an account to access the scanning feature. The same employee may be linked to multiple businesses.

### 3.3 End Customers

End Customers do not need to create an account. They access the service by downloading a Pass to their wallet. Use of the Platform by End Customers is restricted to persons aged 16 or older.

## 4. Description of Service

Stampéo provides businesses with a platform to:

- Create and customize digital loyalty cards
- Distribute cards via Apple Wallet and Google Wallet
- Scan customer cards to award stamps or points
- Track loyalty program activity
- Send notifications to cardholders

The service includes the design of the first loyalty card by our team, based on visual assets provided by the Business User.

## 5. Subscriptions and Pricing

### 5.1 Free Trial

Each new Business User receives a 30-day free trial, with no commitment and no credit card required. The trial provides access to all features of the selected tier.

At the end of the trial, the Business User must subscribe to continue using the Platform. Otherwise, dashboard access is suspended. Passes already installed in customer wallets remain present but are no longer updated.

### 5.2 Subscription Tiers

| Tier | Price | Key Features |
|------|-------|-------------|
| Pay | €20/month excl. VAT | 1 active card, up to 3 scanners, unlimited customers and scans |
| Pro | €40/month excl. VAT | Multiple cards, unlimited scanners, promo campaigns, multi-location, advanced analytics |

Prices are shown excluding VAT. Applicable VAT will be added in accordance with current regulations.

### 5.3 Payment

Payment is made monthly by credit card via our payment processor Stripe. Subscriptions are automatically renewed each month on the anniversary date.

### 5.4 Price Changes

Stampéo reserves the right to change its prices. Any change will be notified by email at least 30 days before taking effect. Business Users who do not accept the change may cancel their Subscription before the effective date.

## 6. Cancellation

### 6.1 By the Business User

Business Users may cancel their Subscription at any time from their dashboard. Cancellation takes effect at the end of the current billing period. Access to the service is maintained until that date.

No pro-rata refunds are provided for the remaining period.

### 6.2 By Stampéo

Stampéo may terminate an account in the event of:

- Violation of these Terms
- Fraudulent or abusive use of the Platform
- Non-payment after reminder
- Extended inactivity (more than 12 months without login or scan)

In case of termination for violation, Stampéo reserves the right to suspend access immediately without refund.

### 6.3 Consequences of Cancellation

After cancellation:

- Dashboard access is deactivated
- Data is retained for 60 days, allowing possible reactivation
- After this period, all data is deleted, including End Customer data
- Passes already installed in customer wallets remain present but are no longer updated

## 7. First Card Design

Stampéo offers to design the first loyalty card for each Business User, based on provided assets (logo, colors, existing physical card, etc.).

The Business User retains full ownership of their brand assets (logo, trade name, visual identity). Stampéo claims no ownership rights over these elements.

The card design produced by Stampéo may be freely used by the Business User within the Platform.

## 8. Intellectual Property

### 8.1 Stampéo's Property

The Platform, its source code, interface, design, and features are the exclusive property of Stampéo. The Subscription grants a non-exclusive, non-transferable right to use the Platform for the duration of the Subscription.

### 8.2 Business User Content

Business Users retain ownership of their content (logo, images, text). By uploading content to the Platform, Business Users grant Stampéo a non-exclusive, worldwide, royalty-free license to use, display, and distribute such content for the purpose of operating the service.

Business Users also authorize Stampéo to showcase their loyalty card and logo as commercial references on the Stampéo website (e.g., testimonials, card examples). Business Users may withdraw this authorization at any time by contacting support@stampeo.com.

## 9. End Customer Data

### 9.1 Responsibility

The Business User is the data controller for their End Customers' data. They determine what information is collected and for what purpose. Stampéo acts as a data processor in accordance with Article 28 of the GDPR.

### 9.2 Business User Obligations

Business Users agree to:

- Comply with the GDPR and all applicable data protection regulations
- Inform their End Customers about data collection and use
- Have a valid legal basis for data processing
- Respect their End Customers' rights (access, rectification, deletion)
- Not use the Platform to send unsolicited communications in violation of applicable regulations

### 9.3 Data Isolation

End Customer data is strictly isolated per business. Stampéo performs no cross-business data matching.

## 10. User Obligations

### 10.1 Business Users agree to:

- Use the Platform in accordance with its intended purpose and applicable laws
- Not resell or sublicense access to the Platform
- Not attempt to circumvent technical limitations of their subscription tier
- Be responsible for the confidentiality of their login credentials
- Ensure that content published via the Platform does not infringe third-party rights

### 10.2 Employees agree to:

- Use the scanner app only within the scope of their duties
- Not share their login credentials
- Respect the confidentiality of End Customer data

## 11. Availability and Maintenance

Stampéo strives to ensure Platform availability 24/7. However, Stampéo does not guarantee uninterrupted operation and shall not be held liable for:

- Interruptions due to maintenance (scheduled or emergency)
- Failures of hosting providers or third-party services
- Force majeure events

The scanner app functions offline: scans are recorded locally and synchronized when connectivity is restored.

## 12. Limitation of Liability

To the extent permitted by law:

- Stampéo provides the Platform "as is" without warranty of fitness for a particular purpose
- Stampéo's total liability shall not exceed the amounts paid by the Business User during the preceding 12 months
- Stampéo shall not be liable for indirect damages, data loss, loss of revenue, or loss of profits
- Stampéo is not responsible for the content of loyalty programs created by Business Users or their commercial relationships with their End Customers

## 13. Right of Withdrawal

In accordance with Article L221-28 of the French Consumer Code, the right of withdrawal does not apply to contracts for the supply of digital content not provided on a tangible medium whose performance has begun with the consumer's agreement.

By subscribing and beginning to use the Platform, the Business User acknowledges and accepts the loss of their right of withdrawal.

However, the 30-day free trial without credit card allows testing the service with no commitment.

## 14. Governing Law and Disputes

These Terms are governed by French law.

In case of dispute, the parties agree to seek an amicable resolution. Failing an amicable agreement within 30 days, the dispute shall be submitted to the competent French courts.

In accordance with applicable regulations, consumer Business Users may use a free mediation service. The competent consumer mediator will be indicated on the Platform website.

## 15. Changes to These Terms

Stampéo reserves the right to modify these Terms. Any substantial modification will be notified by email at least 30 days before taking effect.

Continued use of the Platform after the effective date of modifications constitutes acceptance of the updated Terms.

## 16. Miscellaneous

- If any clause of these Terms is found to be void or unenforceable, the remaining clauses shall remain in effect.
- Failure to exercise a right under these Terms does not constitute a waiver of that right.
- These Terms constitute the entire agreement between the parties regarding use of the Platform.

## 17. Contact

- **Email:** support@stampeo.com
- **Publisher:** Harry Viennot, sole proprietor — France
- **Hosting:** Supabase (Ireland, EU) and OVH (France)
